...
本方案最终态仅到 **C1**：业务侧（StageAssembler 等）通过防腐层 `SharedStageResources` 暴露的语义化 `get*()` 方法访问配置；不允许出现业务代码直接 `@Autowired InfrastructureProperties` 或旧的 `DeploymentConfigLoader`。旧 Loader 仅作为过渡的只读兼容层，不接受新的业务引用（新增代码不得再引用）。

不再采用“保留 1-2 版本供业务继续使用”的策略，改为：
- 旧类保留：仅为平滑迁移的内部兼容与回滚保险，不对业务开放新增耦合。
- 新代码规范：禁止直接依赖旧 Loader；禁止直接注入 Properties（除防腐层）。
- 清理时机：在所有业务调用切换至防腐层语义方法后即可移除（不强制等待两个版本）。

允许直接注入的唯一例外：与业务无关且变更概率极低的横切型基础变量（例如：日志级别、全局轻量开关），并在独立的横切模块暴露；本次范围不包含此类。
